export interface PromptContext { inline?: boolean; mode: 'build' | 'plan' | 'goal'; references: string[]; skills: string[]; goalBudget?: number }
export const emptyContext: PromptContext = { inline: true, mode: 'build', references: [], skills: [] };
...
export interface Settings {
  ...
  codexEnabled: boolean; grokEnabled: boolean;
  codexPath: string; grokPath: string; fontScale: number;
}
export interface ModelOption { id: string; name: string; efforts: Choice[] }
export interface ProviderInfo {
  enabled?: boolean; provider: Provider; path: string; version: string; available: boolean; connected: boolean;
...
export interface Responses {
  usage: UsageInfo;
  responseText: string;
  deleteProject: null; deleteSession: null; editMessage: Session; pickAttachments: Attachment[]; uploadAttachment: Attachment; attachmentPreview: string | null;
...
export type AppEvent = { type: 'transcript-reset'; sessionId: string } | { type: 'changed' } | { type: 'message'; message: Message } |
  { type: 'command'; command: 'new' | 'search' | 'settings' | 'sidebar' | 'open' | 'review' | 'usage' } | { type: 'appearance' } |
...
export const defaultSettings: Settings = { theme: 'system', language: 'system', codexEnabled: true, grokEnabled: true, codexPath: '', grokPath: '', fontScale: 1 };

export interface ContextUsage { used: number; capacity: number | null }
export interface UsageInfo {
 context: ContextUsage | null;
 limits: { name: string; plan: string | null; windows: { usedPercent: number; minutes: number | null; resetsAt: number | null }[] }[];
 error?: string;
}
