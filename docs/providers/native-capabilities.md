...
Codex、Grok 输入框的“子代理”按钮为当前输入启用委派请求，可随草稿、消息和队列保存。主代理负责调用底座的原生工具、等待子任务并汇总；Moose 不自己调度多个模型。关闭开关意味着不额外要求委派，不会禁用 CLI 根据用户文字或项目指令自行使用子代理。底座配置若禁用了子代理，按钮不修改该配置，提示代理说明限制。Pi 暂不提供该入口。
