...
**先看场景：** 设想聊天接口有时返回纯文本，有时还带工具调用。页面需要按请求方法得到正确的参数和返回值提示，这就是这里的“类型推导”；泛型表示会变化的部分，条件类型表示按条件选择结果类型。
...
**先看场景：** 前端拿到的接口 JSON 只是外部数据：TypeScript 写了类型也不会在运行时替你检查。模型升级后若字段变了，直接读取可能报错；类型守卫就是先检查，再让代码安全地使用字段。
...
**先看场景：** 例如模板是“你好，{{name}}”，调用方要提供 name；若模板还支持年龄和默认称呼，就要同时回答“缺变量怎么办”和“传错类型怎么办”。这道题要求把这两层检查写进接口。
...
**先看场景：** 例如 run_123 可以看出是任务 ID，2026-02-30 却虽然像日期，实际并不存在。模板字面量类型只能检查字符串形状，因此要分清编译时约束和运行时校验。
...
**先看场景：** Agent 执行工具时会经历等待、执行、完成或失败。若状态只是任意字符串，代码可能把“已完成”直接改回“执行中”；这题要让数据结构和转移规则一起约束它。
...
**先看场景：** 一条输出可能是文本、图片或音频；每种内容需要的字段不同，却共享 ID 和时间等信息。先弄清“任选一种”和“同时具备多个属性”的区别，再决定用联合还是交叉。
...
**先看场景：** 同一段 AI 输出可以从 SSE、WebSocket 或别的连接送来。业务层关心的是“收到一段文字”或“发生错误”，不应每换一种传输方式就重写一套业务逻辑。
...
**先看场景：** 对话若能分叉，每个节点可能引用子节点，形成递归结构。问题有两层：怎样写出这个类型，以及怎样避免把整棵很深的树都塞进一个复杂类型或对象里。
...
**先看场景：** 页面展示模型选择器时，需要知道每个模型支持什么输入、最多输出多少内容。写在代码里的配置能由 TypeScript 提前检查；运行时从服务端取到的配置还得重新校验。
...
**先看场景：** 有些接口把正文放在 choices[0].delta.content 里。infer 的任务是从这层嵌套类型中“取出 content 的类型”，它不会帮你在运行时读取或验证真实响应。
...
**可以这样答：** 假设 AI 已经生成了“今天天气”，网络突然断开。页面要保留这几个字，恢复连接后只接收后面的内容。如果重连时把原请求再发一遍，可能重复生成文字，甚至重复执行工具。因此要先区分“恢复同一次生成”和“重新发起一次生成”。

我会让服务端给这次生成分配 `runId`，并给每个**文本增量事件**分配连续的 `seq`。客户端记录最后一个已经追加到页面的序号，例如 `lastSeq = 2`。重连时带上 `runId` 和游标，请服务端从第 3 个事件继续补发。服务端必须暂存事件或提供等价的恢复能力；只有客户端记住游标，无法找回断线期间漏掉的文字。

收到事件时，客户端只追加同一 `runId` 下的下一个序号：`seq <= lastSeq` 是重复事件，丢弃；`seq === lastSeq + 1` 才追加并更新游标；如果先收到 `seq = 4` 而 `lastSeq = 2`，先等或请求补发第 3 个事件，不能直接把第 4 个片段接上。这样即使重连后服务端重发第 2 个事件，页面也不会多出一份文字。

断线时保留现有正文并提示“连接中断”，按退避策略尝试恢复，失败到上限后允许用户手动重试。切换会话、主动停止或收到完成事件时关闭连接；鉴权失效先处理登录。原生 `EventSource` 会自动重连，服务端用 `id:` 标记事件后，浏览器可在重连时发送 `Last-Event-ID`；如果用 `fetch` 读取 POST 流，重试、传游标和取消都要自己实现。无论哪种方式，服务端没有补发能力时都不能承诺无缝续传。参考 [SSE 事件格式与重连](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events)。

**Moose 实例：** 上面说的是逐段追加的 `delta`。Moose 的 `mergeMessages` 处理的是**完整消息快照**：同一个消息 ID 只接受 `seq` 更大的版本，用新全文替换旧全文，因此中间少收到一个版本也能显示最新结果。Service 的 `accept` 则会把 `event.delta` 追加到当前文本；如果同一个 `delta` 在这里被接受两次，文字仍会重复。面试时可以用这个区别解释为什么“快照按版本取新”和“增量按序去重”不能混用。见 [workspace](../../src/lib/workspace.ts)、[accept/flush](../../electron/service.ts)。

### 2. 如何在前端实现一个“流式Markdown解析器”，在AI逐字输出过程中实时渲染标题、列表、代码块，并避免标签截断？

**先看场景：** 模型可能先发来三个反引号中的第一个、再发来后两个；此时单独解析每个网络片段，会把尚未写完的代码块误判成普通文字。先累积原文，才有稳定的 Markdown 上下文。
...
**先看场景：** 一次回答可能同时包含说明文字、代码和表格，而且网络会把各部分拆成任意小块。合并前要先知道“这块属于哪一部分”和“它排在第几个”。
...
**先看场景：** 假设用户同时排了翻译和代码生成，代码任务突然需要优先完成。队列仅调整等待中的顺序还不够，因为翻译可能已经占着唯一的执行资源。
...
**先看场景：** 用户正在输入时，另一边的长回答每隔几十毫秒更新一次。若每次更新都让大段 Markdown 重新渲染，输入可能卡顿；这题讨论怎样让交互先响应。
...
**先看场景：** 网络中断或关页后，用户希望回来还能看到已经生成的内容。要先分清“回放已保存的文字”和“让模型从断点继续生成”：前者靠本地缓存，后者还需要服务端支持。
...
**先看场景：** 同时请求两路 AI 不一定会卡住页面，真正可能占用主线程的是解析大量数据或计算摘要。Worker 用来把这些计算移走，主线程仍负责界面。
...
**先看场景：** fetch 一次读到的字节不一定刚好是一条 SSE 事件：一条事件可被拆成几次读取，几条事件也可一起到。先按协议拼出完整事件，才能判断 DONE 或 ERROR。
...
**先看场景：** 用户看到“还需 20 秒”会以为这是可计算的剩余时间。但 AI 往往不知道自己最终会写多少字，所以先说明哪些数据能测量，哪些只是估计。
...
**先看场景：** 聊天页面同时有长期保存的消息、正在运行的任务、用户偏好和临时弹窗。它们更新频率和归属不同；若全塞在同一个全局对象里，读写关系会难追踪。
...
**先看场景：** “保存一个快照”像游戏存档：重新打开时要知道消息写到哪里、任务当时处于什么状态。仅把当前 React 状态 JSON 化，未必能还原服务端正在做的工作。
...
**先看场景：** 聊天、编辑器等子应用都显示当前模型。若各自存一份，用户在一个模块切换后，另一个模块可能仍显示旧模型；需要约定谁拥有这份状态。
...
**先看场景：** 用户点击发送后，界面应立即反馈，不必等模型生成第一字。但这时能确定的是“请求已提交”，并不知道 AI 将回答什么；乐观更新要围绕能确定的事实。
...
**先看场景：** 用户想回到三轮前，继续提问形成另一条路线，就出现分支。若还要把两条路线合并，必须先定义“同一条消息被分别改过”时如何处理。
...
**先看场景：** 断网时，页面可以读本地历史，但新的写入还要等联网后同步。数据缓存、离线待办和冲突处理是不同环节；单装一个查询库不会自动解决全部。
...
**先看场景：** 两个标签页同时打开同一会话：一个改了标题，另一个要及时看到。数据库负责保存，跨标签页通知负责提醒“数据变了”，两者需要配合。
...
**先看场景：** 聊天历史从几十条变成几万条后，每次输入关键词都遍历全部正文会变慢。搜索标题、正文和时间分别需要不同索引，先定搜索范围才知道怎么优化。
...
**先看场景：** 长聊天有两种卡顿来源：屏幕外仍挂着上千条消息，或单条回答本身就有几万字。虚拟列表主要解决前者；后者还要拆分长内容。
...
**先看场景：** 一张图片的缩放和拖动通常只是改变显示方式，不需要每次重新计算像素。只有叠加复杂滤镜等像素工作时，Canvas 或 WebGL 才可能值得引入。
...
**先看场景：** 用户每敲一个字符，若编辑器把整份大文件重新高亮、折叠并查错，就会卡住。关键是只计算变动和可见区域，并丢弃旧版本的迟到结果。
...
**先看场景：** 一页历史消息很多，但用户一次只看屏幕附近。代码块高亮和富文本解析可以等消息接近屏幕时再做，节省首屏工作。
...
**先看场景：** 例如浏览器里要对两句话计算相似度，推理部分可能涉及大量数值计算。WASM 可以承接这段计算，但模型文件加载和数据传递也有成本。
...
**先看场景：** 模型不断产出文字时，发送按钮和输入框仍要立刻响应。调度器要限制每次处理流式文本的工作量，把主线程的空隙留给交互。
...
**先看场景：** 假设第 50 条消息更新了一个字，前 49 条内容没变，却可能因父组件更新而重新渲染。这题要先找出引用变化从哪里传下来，再选择缓存手段。
...
**先看场景：** 文本、图片和表格的准备时间不同。先展示已到的文字与占位，图片加载完再替换，通常比等所有内容齐了再一次显示更快。
...
**先看场景：** 聊天、编辑、可视化由不同团队开发时，都想独立发布，却还要在同一产品里切换和共享登录。微前端方案首先要解决边界、通信和版本兼容。
...
**先看场景：** 前端、服务端和共享类型若放在多个仓库，改一个接口可能要跨仓库同步发布。Monorepo 把代码放一起，但仍要明确包边界和构建依赖。
...
**先看场景：** 第三方想增加一种模型或工具时，若必须修改主应用源码，扩展会变得危险且难升级。插件化需要先规定插件能做什么、如何加载和隔离。
...
**先看场景：** 同一套 SaaS 给不同公司使用：A 公司用蓝色主题，B 公司有自己的域名和功能套餐。页面必须根据可信租户配置展示，同时不能把隔离责任只放在前端。
...
**先看场景：** “对话”“模型”“工具”“知识库”各有自己的数据和规则。领域划分的目的，是让一个模块的概念变化不迫使所有模块一起改。
...
**先看场景：** 输入框提交后，任务、消息列表和通知栏都想知道状态变化。直接让每个模块相互调用会形成耦合；事件可用于传递“发生了什么”。
...
**先看场景：** 两个人同时编辑同一句话，后提交的人不能简单覆盖前一个人的修改。OT 和 CRDT 都在解决并发操作如何合成；先明确共享数据和冲突规则。
...
**先看场景：** 用户在画布上连起“提问→检索→总结”，保存后希望再次运行。配置只是流程图数据；执行还需要校验节点、处理依赖、失败和取消。
...
**先看场景：** 浏览器直接拿服务商密钥调用模型，会把密钥暴露给用户。所谓“前后端分离”仍要让后端负责凭证、计费和权限边界。
...
**先看场景：** IDE 插件与普通网页的生命周期、权限和 UI 能力不同。轻量 SDK 应给宿主一套稳定调用方式，同时把宿主差异藏在适配层。
...
**先看场景：** 模型可能先要求运行工具，等待结果后再决定下一步。这个循环不是单次请求：每一步都要知道任务是否已取消、工具是否超时、结果属于哪次调用。
...
**先看场景：** Token 主要由送进模型的输入和模型生成的输出决定。前端的缓存或懒加载只有在减少实际模型请求、上下文或重复计算时，才会降低 Token 成本。
...
**先看场景：** 回答看起来通顺，不代表正确或有帮助。质量评估先定义要评什么，再让用户反馈能对应到具体回答、模型版本和问题类型。
...
**先看场景：** 模型可能把猜测写得像事实。界面不能靠一个“AI 可能出错”横幅解决问题；要在具体高风险内容附近提供核对线索和修正入口。
...
**先看场景：** 用户发送文本前，页面可以发现明显敏感内容并提醒。但前端代码在用户设备上运行，能被绕过；真正的安全边界仍在服务端。
...
**先看场景：** 同一个产品想比较两种提示词和两种界面。A/B 实验要保证用户稳定落到某一组，否则同一人来回切换，结果无法解释。
...
**先看场景：** 离线模型意味着下载模型文件、在设备上加载并推理，不只是把网络请求换成 WASM。先衡量文件大小、内存和设备性能。
...
**先看场景：** 对话越长，发送给模型的历史越多，最终会超出上下文窗口。要决定哪些原文保留、哪些压缩成摘要，并让用户知道摘要可能丢细节。
...
**先看场景：** 模型产出的 Markdown 是内容，富文本、代码和表格是展示方式。格式化应尽量保留原文，避免一键转换时悄悄改掉用户要复制的内容。
...
**先看场景：** 目录结构要帮人快速找到“界面在哪、业务规则在哪、服务商接入在哪”。按技术类型或按业务功能组织都可以，关键是边界清楚。
...
**先看场景：** 多人写代码时，风格差异和低级错误会拖慢审查。格式化、静态检查和提交信息检查分别解决不同问题，自动检查要放在合适的时机。
...
**先看场景：** 开发者提交前可以只检查改过的文件，CI 再检查整个项目。Husky、lint-staged、Commitizen 各负责流程中的不同一环。
...
**先看场景：** 一次提交从本地到线上需要经过检查、构建和发布。流水线的重点是让失败尽早出现，并确保部署的是通过同一批验证的产物。
...
**先看场景：** “我电脑上能跑”常因为 Node 版本、依赖或系统库不同。容器可固定运行环境，但前端静态资源和服务端进程的部署方式仍要区分。
...
**先看场景：** 用户说“页面慢”，可能是首屏慢、布局跳动，也可能是点发送后模型迟迟没回应。浏览器体验指标和 AI 任务指标要分别采集。
...
**先看场景：** 线上报错时，仅有一句错误文字往往不够定位。还需要版本、页面、请求和任务信息；用户输入可能含隐私，不能直接全部上传。
...
**先看场景：** 为了还原一次失败请求，需要把“用户做了什么、请求何时发出、模型怎样响应”串在一起。结构化日志让这些记录可检索。
...
**先看场景：** 开发构建慢和用户下载包太大是两类问题。Vite 构建优化要分别看依赖预构建、代码分割、缓存和最终产物。
...
**先看场景：** 模型 SDK 更新可能改接口、错误格式或包体积。依赖管理不只是定期升级版本，还要验证现有功能和发布风险。
...
**先看场景：** 模型说“我要调用计算器”时，前端不能把这句话当成已经算出结果。应用要识别工具请求、执行受权的工具，再把结果交还给模型。
...
**先看场景：** “哪个模型更快更便宜”需要按同一种口径统计：从发起请求到首字、到结束，以及成功率和用量。只看平均值容易掩盖慢请求。
...
**先看场景：** 两个用户提问看似相同，也可能有不同权限、上下文和隐私数据。请求合并要先判断哪些工作真的能安全共享。
...
**先看场景：** SSE 只让服务端向浏览器推送；若客户端要在同一连接上实时发中断或工具结果，就要考虑 WebSocket 的双向消息。
...
**先看场景：** AI 回答还没结束时，页面可以先显示已生成文字和当前步骤。进度条若没有真实总量，应该显示“正在进行”，不要编造百分比。
...
**先看场景：** 同时问多个模型，再比较结果，看似只是发起多个请求。真正难点是限制并发、统一结果格式、等待慢模型以及决定投票规则。
...
**先看场景：** “判断设备”可能是为布局、触屏交互或下载不同安装包服务。目的不同，读取的信息也不同，不能只凭屏幕宽度给用户贴“手机”标签。
...
**先看场景：** 假设修一个功能时连续提交了三次小改动，提交记录有些零碎，想在合并前整理成一次。这里改的是 Git 历史，先确认这些提交是否已经被别人基于其上开发。
...
**先看场景：** 用户关页时，普通异步请求可能来不及完成；少量统计事件可以交给浏览器尽力发送。sendBeacon 解决的是“安排发送”，并不提供送达回执。
...
**先看场景：** 长页面左侧有章节目录：点目录要滚到内容，手动滚页面时目录也要高亮当前章节。这两个方向都要处理，才能叫“滚动跟随”。
...
**先看场景：** 埋点是记录用户关键操作供统计分析。若几十条记录都攒到关页才发，浏览器或进程可能先退出；平时批量发送才是主路径。
...
**先看场景：** 用户点击按钮后页面短暂不响应，可能是主线程被一段计算占住。长任务指标先告诉我们“何时卡了多久”，再用性能录制查具体代码。
...
**先看场景：** 浏览器会记录页面导航、绘制、资源加载等时间点。PerformanceObserver 像订阅这些记录的接收器，先选要观察的指标，再决定如何汇总。
...
**先看场景：** 聊天记录从底部往上读，滑到顶部时加载旧消息。旧消息插到列表最前面后，若不补偿滚动位置，用户正在看的那一行会突然跳走。
...
**先看场景：** 标签页可能在后台但任务仍在跑，也可能显示在屏幕上却没有键盘焦点。先说明“活跃”指可见还是可输入，代码才有明确判断条件。
...
**先看场景：** 传 1GB 文件时，可以一次发送，也可以拆成许多块。两者总字节接近，但重试、请求开销和并发方式会改变真实耗时。
...
**先看场景：** 若每片太小，请求数量暴涨；每片太大，一次失败就要重传很多数据。切片数量由文件大小和每片大小一起决定，先选每片大小。
...
**先看场景：** 用户关页面时，浏览器不会等任意异步代码运行完。必须先确定哪些数据必须可靠保存，哪些只是可尽力发送的统计。
...
**先看场景：** PV 是页面浏览次数，请求数是一次浏览中向接口发了多少次请求。例如用户打开一页并发出五个 API 请求，通常是一次 PV、五次请求。
...
**先看场景：** 一段回答很长时，默认只露出前几行并给“展开”。按钮应只在内容确实超过折叠高度时出现，否则短内容也会显示无意义的操作。
...
**先看场景：** 拖一个卡片时要记住按下点与卡片初始位置，移动时用位移更新位置，松开时结束。还要处理指针移出卡片和触摸设备。
...
**先看场景：** 首屏慢可能是脚本、字体或图片中的某一项拖后腿。要统计所有页面的静态资源耗时，就得在每页收集统一格式的资源记录。
...
**先看场景：** “重复请求”可能是用户连点两次，也可能是两个组件同时读取同一数据，还可能是旧搜索结果晚回来。三种情况需要不同处理。
...
**先看场景：** 侧栏展开后，图表容器变窄了，但浏览器窗口并未改变。ResizeObserver 监听的是元素自身尺寸，适合这种场景。
...
**先看场景：** 如果只是要在窄屏换布局，CSS 已经能做。只有业务代码必须显示当前窗口宽高数值时，才需要监听尺寸变化。
...
**先看场景：** 昨天功能正常、今天报错，中间有几十个提交。git bisect 通过反复测试中间提交，把首次引入错误的范围逐步缩小。
...
**先看场景：** 某次提交引入错误时，处理办法取决于它是否已经共享。撤销效果与改写提交历史是两件不同的事，先确认分支使用情况。
...
**先看场景：** 线上用户说“点了发送就出错”，开发者需要知道此前页面、操作、请求与错误的先后顺序。记录操作线索与录屏回放不是同一种成本。
...
**先看场景：** 请求报错时只知道 URL，可能找不到是哪个组件发起的。可在统一请求入口记录调用堆栈，再用与发布版本匹配的 source map 还原源码位置。
...
**先看场景：** 一个页面同时请求十个接口，网络断开后十个请求一起失败。如果每个请求各弹一次错误提示，用户会被十条 toast 淹没。
...
**先看场景：** 一长串 if-else 可能只是在映射类型，也可能承载真正不同的业务流程。先看分支为何存在，再决定是否换成表、函数或状态机。
...
**先看场景：** 编译器把新语法转换为旧环境可运行的代码时，可能需要重复插入辅助函数。babel-runtime 相关工具解决的主要是这类重复代码和运行时依赖。
...
**先看场景：** “预览 PDF”可能只要求点开能看，也可能要求页码、搜索和标注。需求层级不同，浏览器自带查看器和 PDF.js 的成本差很多。
...
**先看场景：** 用户先拖选一句话，再右键希望出现“复制/翻译”。菜单要确认选择属于指定文章区域，不能误把页面别处的选区当成正文。
...
**先看场景：** 浏览器选中文字时，会记录起点和终点，即使文字跨多个标签也一样。富文本编辑器还需要把这个 DOM 位置转为自己的文档位置。
...
**先看场景：** 用户只说“页面不好用”很难定位。监控体系要先知道是否出错、哪里慢、哪项业务失败，再把这些信息关联到一次操作。
...
**先看场景：** 反馈工单里若只有“打不开”，开发者无法复现。入口可自动附上环境与版本，让用户集中补充操作步骤和实际现象。
...
**先看场景：** 设计稿写 24px，而样式系统按 rem 写。换算前先知道根元素字号：rem 是相对根字号的长度单位。
...
**先看场景：** 网页能展示 CDN 图片，却不代表脚本能随意读取 CDN 的响应内容。浏览器对“嵌入资源”和“读取跨源数据”有不同规则。
...
**先看场景：** 登录状态想在 app.example.com 和 admin.example.com 之间共用，首先要看它们是否有共同父域。Cookie 的 Domain 属性决定发送范围。
...
**先看场景：** 搜索框连续输入三个词，前两个请求已不再需要。取消请求能节省资源，但还要防止旧结果在取消前已经返回并覆盖新结果。
...
**先看场景：** 点标题展开更多内容，是一个状态切换。先决定是否需要自定义行为；简单场景浏览器原生 details 就能提供语义和键盘支持。
...
**先看场景：** 点击事件发生在按钮上，代码想确认按钮是否属于某个卡片。DOM 的包含关系可以用 contains 检查，但自身也算包含自身。
...
**先看场景：** 一个对象看似没有字段，却可能在原型上定义了方法。题目把原型链也计入“非空”，所以普通 Object.keys 不能覆盖全部情况。
...
**先看场景：** JavaScript 的“假值”和产品定义的“空”并不相同。这里把 0 也算空，却未必把 false 算空；先列清业务规则再写函数。
...
**先看场景：** 卡片正面是问题，翻面后显示答案。实现时通常同时摆放正反两面，再旋转包住它们的内层容器。
...
**先看场景：** 几个 flex 子项同时写 flex:1 时，它们会分配容器剩余空间。理解它要拆开看增长、收缩和基础尺寸三个值。
...
**先看场景：** 重构是改变代码结构以便维护，目标不应只写“代码更优雅”。先找一个具体痛点，并用现有行为作为校验基线。
...
**先看场景：** 一个函数搜不到直接调用，不代表它一定没用：路由、动态导入或插件注册可能间接引用它。删除前要追入口和运行路径。
...
**先看场景：** 把按钮“发送”翻译成另一种语言只是第一步。日期、数字、复数以及不同语序也会影响用户看见的内容。
...
**先看场景：** 新版本一次推给全部用户，若有问题影响面很大。灰度发布先给稳定的一小部分用户开放，并观察数据后扩大范围。
...
**Moose 边界：** 当前 macOS 桌面与独立 Web 安装版作为同一版本发布；没有自动更新/灰度平台。若设计桌面灰度，应有稳定更新通道、版本签名和兼容存储迁移，不能直接照搬网页百分比路由。

### 46. [微前端]为何通常在微前端应用隔离，不选择iframe方案

**先看场景：** iframe 像把另一个独立网页嵌进当前页，隔离很强。若子应用要像同一页面的一部分那样共享路由、弹层和滚动，整合成本会上升。
...
**先看场景：** 多个子应用都想修改 window 上的变量，容易互相污染。Qiankun 的 JS 隔离就是在子应用运行时控制这类全局读写。
...
**先看场景：** “隔离”可以只是避免变量重名，也可以要求代码在独立执行环境，甚至要求安全边界。不同技术提供的隔离程度不同。
...
**先看场景：** 列表中间插入一条消息后，后面所有项的 index 都变了。React 若把 index 当身份，原来属于某行的输入状态可能跑到另一行。
...
**先看场景：** Context 给一组组件共享值。值变化时，读取它的组件会更新；先确认哪些组件真的读取高频状态，再决定拆分或稳定引用。
...
**先看场景：** 截图可能指截一个 DOM 区域，也可能指真实屏幕或整页长图。范围不同，浏览器权限和还原精度也不同。
...
**先看场景：** QPS 是每秒请求数。高峰时先判断是模型调用、数据库还是网络到达瓶颈，再限制流量或增加处理能力。
...
**先看场景：** JavaScript 的 Number 用浮点数表示数字，超过安全整数后，相邻整数可能变得不可区分。处理前先确认数据要做计算还是只展示。
...
**先看场景：** 同一个链接在电脑和手机打开，可以由同一网页自适应布局，也可以导向两套应用。先确定产品是否真的需要两套入口。
...
**先看场景：** “体验好”太笼统。以发送 AI 请求为例，用户应知道已排队、正在执行、失败原因和如何停止或重试。
...
**先看场景：** 页面一次要加载几百项数据，全部同时请求会压住网络和服务端。需要限定同一时刻运行多少个任务，并决定哪些任务先跑。
...
**先看场景：** 想比较各页面接口快慢，必须从同一时钟起点记录发起、结束、失败和取消。统一封装才能让统计口径一致。
...
**先看场景：** 大文件上传的难点不只是拆块，还包括断点续传、校验、重试和最终合并。先把完整生命周期讲清楚。
...
**先看场景：** 手机屏幕窄、字号设置多样、横竖屏也会变化。移动适配要让布局随容器变化，而不是把整页按比例缩小。
...
**先看场景：** 用户切换明暗主题时，每个组件都要换颜色。统一使用“背景/文字/边框”等语义颜色变量，才能一次切换整站。
...
**先看场景：** 页面首屏加载、路由切换、文件下载和模型生成都会显示“加载中”，却没有同一套真实百分比。先界定进度条代表哪件事。
...
**先看场景：** 长页面有许多屏外图片，初始全下载会拖慢首屏。懒加载让图片接近屏幕时才开始请求。
...
**先看场景：** Cookie 除了名称和值，还带着决定何时发送、能否被脚本读取等规则的属性。回答时可按“作用域、期限、安全”理解这些属性。
...
**先看场景：** 电脑展示二维码，手机扫码确认后电脑登录。二维码应指向一次短期登录会话，而不是直接包含长期账号凭证。
...
**先看场景：** 浏览器访问域名前，要找到目标服务器的地址。DNS 负责这个查询；缓存能减少重复查询，但记录也会按 TTL 过期。
...
**先看场景：** 如果一个函数只根据输入算输出，不改外部东西，就能容易地单独测试。函数式编程常把这类计算与数据库、网络等副作用分开。
...
**先看场景：** 水印可以提醒截图归属，但覆盖层可能被懂技术的用户移除。先说明目标是提示追溯还是强制保护，再选页面或导出时写入。
...
**先看场景：** 做聊天产品时，“会话”“消息”“生成任务”不只是数据库表名；它们有各自身份、关系和约束。领域模型是把这些业务规则明确表达出来。
