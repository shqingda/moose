...
> 根据 `interview.md` 逐题整理，核对当前 Moose 0.5.5 源码。正文共 145 题，按要求跳过 7 题，回答 138 题；保留原章节和题号，方便定位。原文件章节标题中的题数与实际条目不一致，这里按实际题目整理。
>
> 阅读方法：先练每题的“可以这样答”，追问时再讲实现步骤和 Moose 实例。本文提供面试表达模板，不把设计方案当作你的既有经历。代码标为教学示例的部分不是 Moose 源码；没有实际压测支撑的性能数字不作为项目成果。
>
> 跳过：二-10（语音同步）、三-3（三方状态图）、三-6（不可变工具库）、三-9（原子状态库）、四-7（语音流）、六-2（指定推理库）、八-3（指定 AI 链框架）。混合选项题仅讲 Zustand、Sentry 基础、Vite；不展开 Redux、XState、immer/immutable.js、Recoil/Jotai、AI 语音、TensorFlow.js/ONNX Runtime、Bugsnag、Webpack、LangChain/LangGraph。原题为便于对应而原样保留，可能含上述词名。

## 先准备一段 Moose 开场

**30 秒版：** Moose 是一个本地 Coding Agent 桌面客户端，用 React 和 Electron 提供项目、对话、审批和 Git 改动审阅。它通过本机 CLI 接入 Codex 与 Grok，把不同协议转换成统一消息，再存到本地 SQLite。重点不是自己训练或运行模型，而是让长任务的输出、排队、取消和恢复在界面上可靠地工作。

**1–2 分钟版：** 用户先选择项目目录，发送后输入先进入持久化队列。同一个目录里一次只执行一个任务，避免客户端内部的代理同时改同一份文件；不同目录可以并行。后台适配器把文本、工具、审批等事件统一起来，按记录合并文本，约每 80ms 保存并推给前端；前端按 ID 和版本合并，避免旧快照覆盖新结果。数据库和协议处理放在独立运行进程，React 负责展示，受限 preload 负责桥接。程序重启后保存的历史还能看，未完成任务标为中断，队列暂停，避免自动重复执行。长消息通过分页、memo 和屏外渲染优化减少开销，但并没有做完整虚拟列表、离线推理或远程监控平台。
...
**Moose 实例：** [AgentAdapter](../../electron/providers/types.ts) 使用 `emit(AgentEvent)`，把 Codex 与 Grok 的协议统一后交给 Service；这是同样的分层思想，实际传输不是 SSE/WebSocket。
...
**Moose 实例：** 当前按项目真实路径串行，同目录只允许一个 active 任务，不同项目并行；队列按时间处理，**没有优先级抢占**。`stop` 会暂停该会话后续队列并关闭 adapter。扩展优先级时应保留目录互斥规则。清理代码虽写在 `finally`，仍需保证 `adapter.close()` 抛错不会跳过锁释放；“用了 finally”本身不是充分保证。见 [Service](../../electron/service.ts)。
...
**Moose 实例：** 当前重点是 80ms 批量发布、消息级 `memo`、历史分页、后台 utility process；源码没有使用这两个 Hook，面试可说“进一步优化方向”，不能说已经落地。
...
**Moose 实例：** 重业务放到 Electron utility process，使用 IPC 传递请求与事件。它和 Web Worker 都体现“重计算离开界面线程”，但进程隔离、Node 能力、崩溃边界不同，不能混称。
...
**Moose 实例：** 只允许编辑空闲会话的最新用户消息。可用时通过原生 fork 准备前文上下文，否则生成 historySeed；事务替换最后一轮并发出 `transcript-reset`。不创建侧栏分支，也没有任意回退/合并系统。见 [replaceMessage](../../electron/service.ts)。
...
**Moose 实例：** SQLite 是主数据源，Renderer 经 IPC 接收变化；少量展示偏好使用 localStorage。没有 IndexedDB 跨标签页方案，不能把 Electron 多进程同步等同于浏览器标签页同步。
...
**Moose 实例：** 80ms 合并消息更新，审批 pending 会立即 flush，后台执行位于 utility process。这是“不同变化用不同刷新策略”的具体例子；没有自研 React 渲染调度器。
...
**Moose 边界：** 不同项目可并行，但当前没有自动多模型投票/融合，同项目也受目录互斥约束。不能把支持两个 provider 说成已有 ensemble 系统。
...
`beforeunload` 主要用于确有未保存内容时提示用户，不应用来阻塞式发请求；移动端杀进程可能完全不触发。**Moose** 的正常退出有 `_shutdown` 清理路径，强制杀进程仍需依靠 SQLite 和重启状态恢复，而不是假设 close 回调总能跑完。
