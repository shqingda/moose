...
这份文档统一说明 Moose 的项目经历、技术取舍和面试追问，以 0.19.0 为客户端基线，核对日期为 2026 年 9 月 21 日。功能范围以[能力表](../providers/native-capabilities.md)为准，测试方法与发布证据分别链接。

阅读顺序是：先能介绍项目，再能讲清一条任务怎么执行，最后挑几个自己最熟悉的难点展开。文中的第一人称回答供你练习，面试时按实际参与和理解程度讲，不必逐字背。

## 怎么用这份指南

本次排序面向国内前端、AI 应用前端与偏前端的全栈岗位，观察窗口为 **2026-03-21 至 2026-09-21**。P0 是优先准备，不表示每家公司必问；P2 是项目细节，不表示不重要。每个三级标题下标注优先级，不改原有章节顺序，方便按执行链路阅读。
...
**先复习这条线：** 第 1、2、4 节 → 第 6、7 节 → 第 14、15 节 → 第 16 节例子一 → 第 17 节性能、幂等与 AI 辅助开发。第二轮再补第 3、5、8、9、12 节。不要为了介绍全部功能，把前五分钟讲成菜单列表。

**岗位调整：** Electron／IDE 岗把第 3、10、13 节及打包、PATH 问题升为 P0；Agent 平台岗把第 5、8、9、12 节升为 P0；普通前端岗先讲 React、异步竞态、性能和交互，再展开代理协议。RAG、向量检索、模型训练不是 Moose 已实现的能力；职位要求涉及这些时另行准备，不编进项目经历。JS／TS、浏览器网络与算法基础仍要单独复习，这份项目指南不能替代它们。

**国内近半年依据与局限：**

- [牛客：字节 AI 全栈开发一面，08-03 发布](https://www.nowcoder.com/discuss/913848724952997888?sourceSSR=home)：第一人称复盘，涉及 Agent 项目完整链路、流式返回、个人实现范围及 AI 编码出错后的排查。这支持优先准备项目真实性与执行链路，不足以推断全行业频率。
- [牛客：虚拟盒子前端实习，2026-09-17 面试](https://ac.nowcoder.com/discuss/1676918?type=0)：第一人称复盘，包含 SSE、断网、心跳、基础网络和 AI 记忆追问。作者说明使用录音转写和 AI 整理，因此不把措辞视为官方逐字题库。
- [牛客：字节前端 5—6 月面经汇编](https://www.nowcoder.com/discuss/923168327453642752)：补充观察 AI 工作流、Skills／MCP、测试与传统前端基础；属于二次整理，权重低于个人经历。
- [牛客：字节 Agent 9 月面经汇编](https://www.nowcoder.com/discuss/930603820395036672)：补充观察超时、幂等、SSE 重连、长任务耗时分析；同样不是可独立核验的企业题库。

这里只采用国内岗位样本，不引用海外面试变化。窗口内可核对的样本集中在 5—9 月，且偏实习与字节，不能代表全部公司、职级或半年的均匀趋势。优先级是结合这些信号与 Moose 实现作出的准备建议，不是“半年题库已全部换掉”的结论；下次投递应再按具体 JD 校准。
...
>
> 架构上，React 渲染进程负责交互，Electron 主进程负责窗口和系统能力，独立后台进程负责 CLI、数据库和调度。Web 入口复用业务服务，通过 HTTP 和 SSE 与前端通信。安装版会自动连接或启动本机共享后台，桌面和浏览器使用同一实例。原桌面数据目录保持不变，无需复制数据库；普通退出保留任务，显式停止后台才结束任务。
...
目前没有可引用的用户规模、商业收入或提效比例。客户端启动、内存和包体有受限场景下的测量，见第 16 节，不能据此推断用户生产效率。面试时可以讲完成了什么、验证了什么，以及为什么这么设计，不需要给个人项目补一个没有依据的增长指标。
...
用户手动启动的后台命令由 Moose 管理，因此不能笼统地说“所有命令都由底座执行”。用户在命令面板里启动的进程，和 Codex 工具内部创建的进程，是两套不同的生命周期。
...
React 负责页面状态和交互。主进程负责窗口、菜单、文件选择、剪贴板和打开系统浏览器。独立后台负责服务调度、数据库、代理连接以及后台命令。开发与独立模式仍使用 `utilityProcess`，安装版默认使用脱离客户端生命周期的本机服务。Preload 是页面调用主进程的桥接脚本，不是第四个独立业务进程。

同步数据库操作在后台进程执行，不直接阻塞 Electron 主进程的事件循环。窗口关闭后，任务能否继续取决于后台进程是否仍在运行；窗口重建时通过快照和消息记录恢复界面。安装版普通退出只断开客户端；停止共享后台才会停止任务。独立开发模式仍随应用退出停止后台。
...
安装版按需启动只监听本机回环地址的共享后台，桌面与浏览器复用原桌面数据目录。源码启动的独立 Web 工作区保留单独数据目录，也支持通过连接文件显式接入。访问链接携带一次启动有效的令牌，换取 HttpOnly 会话 cookie 后清除地址中的令牌。服务校验 Host、Origin 和业务参数；客户端断开不触发服务关闭。

浏览器重连时重新读取快照与消息，消息按 ID 和版本合并；写请求不会自动重试，因为响应丢失不代表命令没执行。终端输出通过 IPC／SSE 推送，初次打开和重连按输出游标补读。补读与实时输出可能重叠，因此按游标去重；客户端渲染繁忙时合并为一次补读，不逐块积压。缓冲有上限，超出保留范围会明确提示截断。终端输入和尺寸调整采用单控制端租约：多个窗口可以查看，一个窗口负责输入，其他窗口可主动接管；旧控制端的后续写入由服务端拒绝。关闭面板释放租约，异常断线则在 15 秒后失效。控制权不决定 Shell 是否继续运行。高延迟输入优化和完整业务事件游标仍是后续工作，不能说已经实现无缝远程协作。

这里没有另写一套数据迁移系统。安装版继续使用原来的 SQLite 和附件目录，只改变后台进程的启动方式；桌面读取私有连接文件，通过本机服务操作这些数据。重新打开桌面会复用服务，升级遇到旧版后台则要求先停止再重启。已有独立 Web 工作区不会自动合并，也没有安装系统守护服务。这样减少了双份数据同步和迁移失败后的恢复成本。
...
这张表描述 Moose 当前已接入的范围（截至 0.17.0），不是对各个底座全部能力的判断。入口会结合底座返回的能力和已验证接口决定是否开放；服务端也检查条件，不能只靠隐藏按钮。
### 审批按钮怎么找到原来的请求
...
> 我先区分客户端断开和执行进程死亡。桌面或浏览器只是界面，共享后台拥有数据库、代理进程和终端。关掉窗口后，只要后台还活着，任务就能继续；界面重新连接时读取快照、持久化消息，并补读终端输出。后台已经崩溃，则只能核对记录并恢复到可继续操作的状态，不能凭会话 ID 恢复已死亡的 Shell。
>
> 对写操作最危险的是“请求已执行，但响应丢失”。这种情况我标记为结果未知，不自动重试。审批也不能因为重新连接就默认批准。升级遇到旧版后台则拒绝业务写入，提示先停止旧后台再重开，避免新旧协议混用。
>
> 验证上，先用夹具构造断连和重启时序，再做少量真实底座任务。已经验证 Pi／OpenCode 的文件写入、跨进程续接和运行中取消，以及 OpenCode 审批批准和拒绝。真实系统休眠、数小时内存趋势和实际安装替换仍未验收，我不会把短时自动化通过说成长期稳定性已经保证。

**如果追问“重连是否把所有事件逐条重放”：** 不是。当前依靠状态快照、持久化消息和终端游标补读重建界面，没有通用的全量事件日志。流连接恢复也不等于业务结果一定已知，尤其不能从 SSE 自动重连推导出写操作可以安全重试。

**如果追问“一个任务从五分钟变成二十分钟怎么排查”：** 先确认慢在排队、CLI 启动／握手、模型首个输出、工具执行、待审批还是 UI 更新，再比较相同底座、模型和输入的结果。界面卡顿用 renderer 性能工具，进程内存分别看前端与后台，工具迟迟不退出则检查对应子进程。当前没有统一跨进程 trace 或完整阶段耗时仪表盘，不能把这套排查方法说成产品已具备的观测系统。

**可引用的测量：** 2026-09-21，一万条历史、20 次会话打开与分页，66 秒内 DOM 没有累积；第 5～20 轮 GC 后堆从 10.44 增到 10.93 MiB，增长趋缓。它只说明这个短时、强制 GC 的 renderer 场景，不能证明整个应用无内存泄漏。详见[测试记录](../testing.md#长期运行与恢复当前覆盖与待验收)。
...
Grok 则通过 ACP 发送底座自己的 `/goal` 命令。Moose 没有在外面自己写一个“回答完就自动再问一次”的循环，也不能把 Codex 的预算和状态语义直接套给 Grok。Pi 与 OpenCode 尚未接入 Goal。
...
**本节优先级：P1 · 能展开；Electron／IDE 岗升为 P0。**
...
Moose 0.11.0 先接入 Codex 管理接口。用户可以看配置来源、修改用户级默认模型、切换 MCP／插件、安装卸载插件、注册 MCP 和发起 OAuth。Hooks 先做只读诊断。其他底座继续使用各自 CLI 管理，不能把 Codex 的插件格式直接拿过去用。
...
安装版关闭窗口或普通退出时，共享后台仍在，命令与终端继续运行；停止共享后台才清理其管理的进程。默认独立开发模式随应用退出停止。后台被突然杀掉时，专用父进程管道关闭，监视逻辑据此清理它拥有的命令进程组。
...
固定间隔按经过时长推进，日历规则则按指定时区的每天／每周钟点计算，并预览未来执行时间。两者不能混用：每隔 24 小时不等于夏令时切换后仍在当地每天九点执行。当前没有月度规则或任意 cron 表达式。

安装版普通退出后，共享后台仍可触发调度，但它不是开机自启或系统唤醒服务。电脑睡眠期间不能保证按时执行；重启后需要重新启动 Moose，再按持久化状态核对，不恢复原来的活进程。
...
0.19.0 发布记录为 **166 项单元测试、48 项桌面／Web 回归通过**，并通过类型、lint、格式和安装包检查。数字是该版本的通过用例数，不是覆盖率，也不表示没有缺陷。真实 CLI 控制验收单独记录，不计入上述回归数量。

真实 Codex 已有原生 Plan、按修改后的计划执行、同回合插话的最小验证。Codex 和 Grok 分别有一次子代理计算 `2 + 2` 并返回 `4` 的真实委派验证，也验证过真实历史读取。2026-09-21 又验证了 Pi 0.85.1／`openai-codex/gpt-5.6-luna` 与 OpenCode 2.0.10／`xai/grok-4.6` 的真实写文件、跨进程续接和取消；OpenCode 的真实审批批准／拒绝也通过。Pi 没有内置审批沙箱，不能声称已验收同等权限隔离。
...
0.16.1 继续把历史、worktree 和计划面板改为按需加载，Git 审阅模块延后到选中项目；历史与 worktree 共用一个弹窗外壳。这里按已有职责拆加载边界，没有增加通用面板框架。安装包仅保留产品支持的英语、简体中文系统资源及对应变体，保留 Unicode 数据和运行组件。

同一台机器、同一测量脚本下，与 0.16.0 比较如下。JS 包括首屏入口和 HTML 预加载的全部依赖，不能只拿拆分后变小的 index 文件比较。
...
`pnpm perf:measure` 使用打包应用、临时空工作区、关闭代理发现。窗口隐藏，每次停止并重新启动共享服务，在欢迎界面就绪后等待 1.5 秒采集内存。耗时包含自动化连接开销；RSS 合计可能重复计算共享页。三次样本只能说明本次结果：包体和首屏代码明显减少，启动速度没有可确认的改善，常驻内存只小幅下降。旧版独立后台的测量没有包含共享服务，因此不能直接拼接成趋势。

0.16.2 将 Electron 升至 44.4.3 后，用相同脚本复测三次：启动中位数 632 ms，RSS 合计中位数 562 MiB。与 0.16.1 相比没有显著启动加速，因此不能把上游 ASAR 性能改进直接换算成整个应用的启动收益。

内存优化还要区分常驻量和临时分配。原来的终端恢复把全部历史输出查到 JS，再反序列化，只为把运行状态改成未知。我改为在 SQLite 内更新状态，删除了批量读取助手。用隔离数据库放入 30 条各 1 MiB 的历史、其中 1 条运行中，在独立进程各测三次：旧逻辑的 JS 堆增量约 54 MiB，新逻辑不足 1 MiB；RSS 增量中位数约 68 → 34 MiB。SQLite 仍需要解析 JSON，这个修改没有消除全部原生内存开销。它是合成场景下的恢复阶段结果，不是整个应用少占 34 MiB 的承诺。

后续如果要继续优化，应测真实长会话下的时间线渲染、终端输出和后台任务，而不是继续盲目拆包。当前没有这些场景的性能瓶颈证据，也没有引入虚拟列表、缓存层或更换桌面框架。
...
交互式 PTY 已实现；终端事件推送与重连补偿也已实现；多客户端输入和尺寸控制权已接入；安装版默认共享后台也已接入；后续重点是长期运行、更多底座原生能力和必要的系统后台管理。自动更新、公证和更多平台则属于分发工作。它们是后续计划，不能列进已经交付的成果。
...
复习时以当前源码和能力表为准。尤其分清原生会话分叉与 Git worktree、界面重连与进程恢复、客户端退出与停止后台；这几组概念影响实际使用，也最容易在追问中混淆。
