...
浏览器重连时重新读取快照与消息，消息按 ID 和版本合并；写请求不会自动重试，因为响应丢失不代表命令没执行。终端首版仍用现有输出游标轮询。高延迟输入优化、完整事件游标和多设备终端控制权仍是后续工作，不能说已经实现无缝远程协作。
...
交互式 PTY 已实现；后续重点是流式终端传输、多客户端输入控制、日历调度及更多底座原生能力。自动更新、公证和更多平台则属于分发工作。它们是后续计划，不能列进已经交付的成果。
