...
这份指南按 0.11.0 的代码和发布记录整理，核对日期为 2026 年 9 月 19 日。它接着原来的[项目经历材料](project-experience.md)往下讲，补上 Pi、原生计划与插话、历史管理、子代理、worktree、Git 提交与审查、配置管理和后台调度。
...
5. [三个代理如何接进同一套界面](#5-三个代理如何接进同一套界面)
...
> Moose 是我做的一款 Mac 桌面应用，把 Codex、Grok Build 和 Pi 放在同一个界面里。用户选一个代码项目，提出需求，就能看执行过程、处理审批，再检查和提交代码改动。
>
> 技术上用了 Electron、React、TypeScript 和 SQLite。模型和代理工具由本机安装的 CLI 提供，我做的是上面的交互、任务管理和数据保存。
>
> 难点主要是任务会持续运行，还会改本地文件。我需要处理三个代理的协议差异，避免同目录任务互相覆盖，也要让用户在停止或重启后知道任务进行到哪一步。后续补了计划审阅、运行中插话、独立工作目录和定时任务。

讲完先停。面试官如果对协议接入感兴趣，就顺着第 5 节展开；如果问并发和可靠性，就讲第 7、10、13 节。开场不需要把所有功能、所有库都列一遍。

### 对方让你展开时

> 我最早围绕一条完整的使用流程做：选择项目，发送需求，看 AI 操作，最后检查文件改动。
>
> 为了减少后台操作对窗口的影响，我把 React 页面、Electron 主进程和后台工作进程分开。页面发出业务请求，后台负责调用 CLI、访问数据库和调度任务。不同代理各有自己的适配器，最后都转换成统一的消息、工具记录和审批卡片。
>
> 单轮任务能跑通之后，我开始处理持续使用的问题。比如同一个目录里开两个会话，不能让它们直接同时写代码；需要并行时，就给它们各自创建 Git worktree。用户想先看方案，我接入 Codex 原生 Plan，再做计划版本和批准执行；用户执行到一半改主意，则通过原生插话接口把要求送进当前回合。
>
> 后面我把 Git 提交、原生审查、配置和后台任务也接进来了。这些功能都涉及真实副作用，所以我会在执行前保存操作意图，检查用户看到的版本有没有变化。请求断开时，如果不能确认是否执行成功，就显示结果未知，让用户先检查现场。
>
> 验证分成业务测试、真实 Electron 流程、少量真实 CLI 验证和安装包检查。0.11.0 的记录是 123 项单元测试和 33 项桌面回归通过，另外实际启动了打包应用，检查数据库、后台命令和定时任务编辑。真实服务商调用和模拟协议测试的覆盖范围，我会分开说明。

### 项目的使用价值

终端已经能调用代理。Moose 提供的价值，是把一次编程任务周围的操作放在一起：项目和会话管理、输入与审批、运行状态、代码改动、提交和历史。多开几段对话以后，用户仍然能知道哪段对话在什么目录里工作，哪些输入还在排队，哪些操作需要自己处理。

这也决定了项目的技术重点。它不是只把回答显示成聊天气泡，还要管理持续运行的本地进程，以及进程对文件、Git 和配置产生的影响。

目前没有可引用的用户规模、商业收入、提效比例或性能对照数据。面试时可以讲完成了什么、验证了什么，以及为什么这么设计，不需要给个人项目补一个没有依据的增长指标。

## 2. 哪些是自己做的，哪些来自底座

这里的“底座”，指 Codex、Grok Build、Pi 及它们提供的程序接口。先分清职责，后面的回答就不容易说大。
...
项目是本地桌面应用。代码目录、聊天数据和调度记录保存在本机，模型请求仍可能通过 CLI 发往服务商。本地存储不等于离线推理，也不等于代理读取的内容永远不出电脑。
...
界面需要流畅响应键盘和滚动，主进程要处理窗口及系统入口，后台又要读数据库、调用 Git、维持 CLI 连接。把这些全堆在一个地方，故障和阻塞就很容易互相影响。
...
这个拆分有两个实际收益。第一，同步 SQLite 查询不会直接占住主进程的窗口操作。第二，用户关掉窗口后，任务仍能由后台继续管理，重新打开窗口再读取状态即可。

它也有成本：原来一次函数调用，现在要处理跨进程传参、超时、退出和状态同步。Moose 用请求 ID 关联返回值，用事件持续推送进度。后台异常退出时，会拒绝还在等待的请求并通知页面，避免按钮一直转圈。

### TypeScript 和 Zod 为什么都需要

TypeScript 让开发时的方法名、参数和返回类型对应起来。例如请求 `scheduleUpdate` 时，必须传任务 ID、版本和允许修改的字段。

程序真正收到 IPC 数据后，还要用 Zod 检查。运行时的数据不会因为写过类型声明就自动合法，所以 UUID、枚举、字符串长度和额外字段都需要验证。主进程还会检查请求是不是来自可信窗口的主 frame。类型约束、输入校验和来源检查各管一层。
...
三个 CLI 的启动方式、消息格式和完成信号都不同。Moose 保留三个适配器，让它们各自理解原协议，向上输出统一的 `AgentEvent`。
...
统一事件描述“这是一段回答”“这是工具输出”“这里需要审批”，不要求页面理解三个底座的原始字段。新增 Pi 时，共用了按行传输、请求关联和进程清理，但为 Pi 单独实现消息格式转换；不能因为都传 JSON，就把 Pi RPC 当成 Codex 的 JSON-RPC。

### 为什么不把三个执行循环写成一个大函数

可以统一的是稳定的共同部分，例如输入、输出类型和生命周期接口。不能强行统一的是协议语义：Grok 恢复会话可能回放历史，Pi 的一次结束事件之后可能继续重试或压缩，Codex 又有 thread、turn 和子线程的区分。
...
| 能力 | Codex | Grok Build | Pi |
| --- | --- | --- | --- |
| 普通对话与原生会话恢复 | 已接入 | 已接入 | 已接入 |
| 权限选项 | 请求批准、原生自动审核、完全访问 | 请求批准、完全访问 | 仅完全访问，无内置审批沙箱 |
| Plan 审阅后执行 | 已接入原生模式 | Moose 未接通此流程 | 未接入 |
| Goal | 原生目标接口 | ACP 转交原生 `/goal` 命令 | 未接入 |
| 当前回合插话 | 原生 `turn/steer` | 保持普通队列 | 保持普通队列 |
| 子代理 | 活动与独立历史面板，控制受能力限制 | 工具活动展示 | 未接入专门管理 |
| 原生历史导入 | 支持 | 支持回放导入 | 未接入浏览导入入口 |
| 显式分叉、手动压缩、原生审查 | 已接入 | 未接入 | 未接入 |
| 配置、MCP、插件管理 | 已接入首批功能 | 使用原 CLI | 使用原 CLI |

这张表描述 Moose 0.11.0 的接入范围，不是对三个底座全部能力的判断。入口会结合底座返回的能力和已验证接口决定是否开放；服务端也检查条件，不能只靠隐藏按钮。
...
普通发送表示“当前任务结束后再处理”。插话表示“把这条补充送给正在运行的当前回合”。Moose 对 Codex 调用 `turn/steer`，带上 `expectedTurnId`，明确指定本次输入属于哪一轮。
...
这不是完整终端。当前没有 PTY，不能承诺支持 vim、top 或依赖终端交互能力的程序。Moose 管理的是自己启动的命令，也不会接管 Codex 已经创建的工具进程。
...
第二层是业务约束。目录归属、审批生命周期、版本校验和互斥都由后台检查。UI 禁用按钮是提示，不能成为唯一防线。普通 Git 等程序使用参数数组调用；用户明确输入 Shell 脚本的命令面板，则按它本来的用途执行脚本。
...
> 我碰到的一个问题是，请求回复和推送通知来自同一连接，但到达顺序不能按业务直觉假设。后来我用明确的回合 ID 判断状态，避免迟到的启动回复把已完成任务重新标成运行。这也让我把测试重点从“接口返回了什么”扩展到“几个事件换个顺序会怎样”。
...
> 这个功能让我意识到，确认动作必须对应一份具体内容。用户同意的是刚才看到的版本，不能把一次确认永久复用到后来变化的状态。计划批准、配置保存和定时任务编辑也都用了类似的版本检查，但各自保留自己的业务规则。
...
> 我没有把恢复做成“重启后统一再试一次”，而是先查有哪些事实已经落盘。命令结果、队列项和调度状态保存的时间不同，恢复时需要把它们重新对应起来。这样用户看到的状态有依据，也能减少重复执行。
...
完整 PTY、日历调度和 Grok／Pi 的更多原生能力是产品补齐方向；自动更新、公证和更多平台则属于分发工作。它们是后续计划，不能列进已经交付的成果。

## 18. 简历写法、演示和最后复习

### 简历可以这样写

**Moose｜个人独立项目｜Coding Agent 桌面客户端**

技术栈：Electron、React、TypeScript、SQLite／Drizzle、Vite、Vitest、Playwright。

- 接入 Codex、Grok Build 与 Pi 的程序接口，将流式回复、工具活动、审批和提问转换为统一事件，复用会话界面并保留底座能力差异。
...
- 0.11.0 通过 123 项单元测试、33 项 Electron 回归及 arm64 安装包验证，覆盖协议事件、目录互斥、取消、冲突和恢复场景。

这段可以按简历空间压成三四条。项目时间和个人职责填写实际情况，放在个人项目经历中；不要把本地后台写成云端服务，也不要把底座模型能力写成自己训练或研发的成果。
...
| 三个底座功能完全统一 | 共用交互和事件结构，能力按实际接入区分 |
...
| 三个底座的差异 | [providers/types.ts](../../electron/providers/types.ts) 及三个适配器 | 共用什么，哪些语义需要单独实现 |
