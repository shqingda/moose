...
这份文档统一说明 Moose 的项目经历、技术取舍和面试追问，以 0.17.0 为基线，核对日期为 2026 年 9 月 20 日。功能范围以[能力表](../providers/native-capabilities.md)为准，测试方法与发布证据分别链接。
...
同步数据库操作在后台进程执行，不直接阻塞 Electron 主进程的事件循环。窗口关闭后，任务能否继续取决于后台进程是否仍在运行；窗口重建时通过快照和消息记录恢复界面。安装版普通退出只断开客户端；“退出并停止后台”才会停止任务。独立开发模式仍随应用退出停止后台。
...
安装版关闭窗口或普通退出时，共享后台仍在，命令与终端继续运行；选择“退出并停止后台”才清理其管理的进程。默认独立开发模式随应用退出停止。后台被突然杀掉时，专用父进程管道关闭，监视逻辑据此清理它拥有的命令进程组。
...
真实 Codex 已有原生 Plan、按修改后的计划执行、同回合插话的最小验证。Codex 和 Grok 分别有一次子代理计算 `2 + 2` 并返回 `4` 的真实委派验证，也验证过真实历史读取。Pi 完成了真实 RPC 握手；OpenCode v2 完成了握手与模型发现。现有记录没有验收这两者的真实模型生成。
