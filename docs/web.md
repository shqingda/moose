...
Web 入口复用桌面版的 React 界面和业务服务，在浏览器里管理 AI 编程代理、项目、审批与终端。安装版从 0.16.0 起自动启动本机共享后台，可从 Moose 菜单直接在浏览器打开同一个工作区。

## 安装版：直接使用

打开 Moose 即可。后台按需启动，沿用原来的桌面数据目录，项目、会话、设置和附件无需搬迁。

- 菜单 **Moose → 在浏览器中打开**：打开当前后台的网页入口。
- **退出 Moose（后台继续运行）**：关闭桌面客户端，任务和终端继续运行。
- **退出并停止后台**：结束后台任务和终端，关闭服务后退出桌面。
...
需要回退独立模式时，先从菜单停止共享后台，再设置 `MOOSE_RUNTIME_MODE=local` 启动。同一目录存在共享后台锁时，独立模式拒绝打开数据库。异常退出留下的锁不会自动删除，也不会按旧 PID 杀进程；先核对数据目录的 `runtime.log` 和实际进程状态。
