...
export function createWebAPI(chooseProject: () => Promise<string | null>): MooseAPI {
  ...
  const keyboard = (event: KeyboardEvent) => {
    ...
    let command: Extract<AppEvent, { type: 'command' }>['command'] | undefined;
    if (event.ctrlKey && !event.metaKey && event.code === 'Backquote')
      command = event.shiftKey ? 'new-terminal' : 'terminal';
    else if (event.metaKey || event.ctrlKey) {
      const key = event.key.toLowerCase();
      if (event.shiftKey)
        command = ({ b: 'review', j: 'commands', s: 'schedules' } as const)[key as 'b' | 'j' | 's'];
      else
        command = (
          { n: 'new', k: 'search', b: 'sidebar', o: 'open', u: 'usage', ',': 'settings' } as const
        )[key as 'n' | 'k' | 'b' | 'o' | 'u' | ','];
    }
    if (command) {
