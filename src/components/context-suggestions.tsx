...
export function useSuggestions(
  ...
  useEffect(() => {
    ...
    const dismiss = (event: PointerEvent) => {
      if (
        ...
        (!event.target.closest('.context-menu') && event.target !== input.current)
      )
        setDismissed(true);
    };
  ...
  const rows = [
    ...
    ...entries.map((item) => ({ ...item, mode: undefined })),
  ];
  /** 将选中项插入当前光标位置，同时维护文件或技能引用信息。 */
  const choose = (at: number) => {
    ...
    onContext(
      row.mode
        ...
        : row.kind === 'skill'
          ? { ...context, skills: [...new Set([...context.skills, row.id])] }
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (['ArrowDown', 'ArrowUp'].includes(event.key)) {
      ...
      setIndex(
        (value) =>
          (value + (event.key === 'ArrowDown' ? 1 : -1) + Math.max(rows.length, 1)) %
          Math.max(rows.length, 1),
      );
      return true;
    }
    if ((event.key === 'Enter' || event.key === 'Tab') && rows.length) {
      event.preventDefault();
      choose(index);
      return true;
    }
  ...
  useEffect(() => {
    if (open)
      document.getElementById(`context-option-${index}`)?.scrollIntoView({ block: 'nearest' });
  }, [index, open]);
  const menu = open && (
    <div
      ...
      aria-label={trigger === '@' ? t('projectFiles') : t('commandsSkills')}
    >
      {rows.map((row, i) => (
        <div key={row.id}>
          {(i === modes.length || (i > modes.length && row.scope !== rows[i - 1]?.scope)) &&
            row.kind === 'skill' && (
              <p>{t(row.scope === 'project' ? 'projectSkills' : 'globalSkills')}</p>
            )}
          <button
            id={`context-option-${i}`}
            role="option"
            aria-selected={i === index}
            aria-disabled={row.mode === 'plan' && provider !== 'codex'}
            tabIndex={-1}
          ...
          >
            ...
            <small>
              {row.kind === 'file' || row.kind === 'folder' ? row.path : row.description}
            </small>
          </button>
        </div>
      ))}
      {loading && <p>{t('loading')}</p>}
      {!loading && !rows.length && <p>{t('noContextResults')}</p>}
    </div>
  );
