...
import { Check, ChevronRight, Copy, Terminal, ShieldCheck, Brain, CircleAlert, Pencil } from 'lucide-react';
...
const TranscriptRow = memo(function TranscriptRow({ message, onError, onEdit, busy, latestUser, lastAssistant }: { message: MessageData; onError(error: string): void; onEdit(message: MessageData, text: string): Promise<void>; busy: boolean; latestUser: boolean; lastAssistant: boolean }) {
  const t = useI18n(); const [copied, setCopied] = useState(false), [editing, setEditing] = useState(false), [edited, setEdited] = useState(message.text), [sending, setSending] = useState(false);
  ...
  if (editing) return <div className="message-inline-edit">{!!message.attachments?.length && <AttachmentList items={message.attachments} />}<Textarea autoFocus aria-label={t('editMessage')} value={edited} disabled={sending} onChange={e => setEdited(e.target.value)} onKeyDown={e => { if (e.key === 'Escape' && !e.nativeEvent.isComposing && !sending) setEditing(false); }} /><div className="inline-edit-actions"><Button variant="secondary" disabled={sending} onClick={() => setEditing(false)}>{t('cancel')}</Button><Button disabled={sending || busy || (!edited.trim() && !message.attachments?.length)} onClick={() => { setSending(true); void onEdit(message, edited).then(() => setEditing(false)).catch(error => onError(String(error))).finally(() => setSending(false)); }}>{t('send')}</Button></div></div>;
  return <Message className={`message-${message.kind}`} align={message.kind === 'user' ? 'end' : 'start'}><MessageContent className="message-body"><Bubble variant={message.kind === 'user' ? 'secondary' : 'ghost'} align={message.kind === 'user' ? 'end' : 'start'}><BubbleContent>{!!message.attachments?.length && <AttachmentList items={message.attachments} />}{message.kind === 'user' ? <div className="user-text">{message.text}</div> : <Markdown text={message.text || '…'} onError={onError} />}</BubbleContent></Bubble>{message.state !== 'running' && (message.kind === 'user' || lastAssistant) && <div className="message-actions"><IconButton size="icon-xs" label={copied ? '✓' : t('copy')} onClick={() => { void (async () => { const text = message.kind === 'assistant' ? await window.moose.request('responseText', { sessionId: message.sessionId, runId: message.runId }) : message.text; await window.moose.request('copyText', { text }); })().then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }).catch(error => onError(String(error))); }}>{copied ? <Check /> : <Copy />}</IconButton>{message.kind === 'user' && latestUser && <IconButton label={t('editMessage')} disabled={busy} onClick={() => { setEdited(message.text); setEditing(true); }}><Pencil /></IconButton>}<time className="message-time" dateTime={new Date(message.createdAt).toISOString()}>{new Intl.DateTimeFormat('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false }).format(message.createdAt)}</time></div>}</MessageContent></Message>;
});
export function Transcript({ session, onError, onEdit }: { session: Session; onError(error: string): void; onEdit(message: MessageData, text: string): Promise<void> }) {
  ...
  const lastAssistants = new Map(messages.filter(m => m.kind === 'assistant').map(m => [m.runId, m.id]));
  return <MessageScrollerProvider autoScroll defaultScrollPosition="end"><MessageScroller className="transcript"><MessageScrollerViewport><MessageScrollerContent className="transcript-content">
    ...
    {messages.filter(message => message.kind !== 'reasoning' || message.text.trim()).map(message => <MessageScrollerItem key={message.id} messageId={message.id} scrollAnchor={message.kind === 'user'}><TranscriptRow message={message} latestUser={message.id === latestUser} lastAssistant={lastAssistants.get(message.runId) === message.id} onError={onError} onEdit={onEdit} busy={['running', 'waiting', 'queued'].includes(session.status)} /></MessageScrollerItem>)}
