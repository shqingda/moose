...
export function Composer({ projectId, session, options, provider, providers, draft, onDraft, onProvider, onOptions, onSend, onStop, onError, disabled, attachments, onAttachments }: {
  ...
  const [skills, setSkills] = useState<ContextEntry[]>([]), [skillsReady, setSkillsReady] = useState(false);
  const migrated = useRef(false);
  useEffect(() => { let live = true; void window.moose.request('listSkills', { projectId }).then(items => { if (live) { setSkills(items); setSkillsReady(true); } }).catch(error => onError(String(error))); return () => { live = false; }; }, [projectId, onError]);
  ...
  useEffect(() => {
    ...
    if (!context.inline) {
      ...
      const missing = [...context.references.filter(path => !present.references.includes(path)).map(path => referenceText('@', path)), ...context.skills.filter(id => !present.skills.includes(id)).flatMap(id => { const skill = skills.find(s => s.id === id); return skill ? [referenceText('/', skill.name)] : []; })];
  ...
  const send = async () => { if ((!draft.trim() && !attachments.length) || sending || disabled || session?.archived || (info?.images === false && attachments.some(a => a.mime.startsWith('image/')))) return; setSending(true); try { if (await onSend(contextInText(draft, context, skills))) setContext({ ...context, skills: [], references: [] }); } finally { setSending(false); } };
  ...
  return <div className="composer-region" onDragOver={e => { e.preventDefault(); }} onDrop={e => { e.preventDefault(); if (!session?.archived) void receive(Array.from(e.dataTransfer.files)); }} onPaste={e => { if (e.clipboardData.files.length && !session?.archived) { e.preventDefault(); void receive(Array.from(e.clipboardData.files)); } }}>
    ...
    {suggestions.menu}
    <InputGroup className="composer-input">{context.mode !== 'build' && <div className="context-chips"><button onClick={() => updateContext({ ...context, mode: 'build' })}>{t(context.mode === 'plan' ? 'planMode' : 'goalMode')}<X /></button></div>}{attachments.length > 0 && <AttachmentList items={attachments} onRemove={id => onAttachments(attachments.filter(a => a.id !== id))} />}
      ...
      <InputGroupAddon align="block-end" className="composer-controls">
        ...
        <div className="flex shrink-0 items-center gap-1"><UsagePanel provider={provider} sessionId={session?.id} />{busy && <IconButton label={t('stop')} onClick={onStop}><Square fill="currentColor" /></IconButton>}<Button size="icon" className="send-button" aria-label={t('send')} onClick={() => { void send(); }} disabled={info?.enabled === false || disabled || unsupportedImages || (!draft.trim() && !attachments.length) || sending || session?.archived}><ArrowUp /></Button></div>
      </InputGroupAddon>
    </InputGroup>
