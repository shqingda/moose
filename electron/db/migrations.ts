...
export function migrate(db: Database.Database) {
  ...
  if (version < 2) db.transaction(() => { db.exec("ALTER TABLE sessions ADD COLUMN draft_attachments TEXT NOT NULL DEFAULT '[]'; ALTER TABLE sessions ADD COLUMN history_seed TEXT NOT NULL DEFAULT ''; ALTER TABLE queue ADD COLUMN attachments TEXT NOT NULL DEFAULT '[]'; PRAGMA user_version = 2;"); })();
  if (version < 3) db.transaction(() => { db.exec("ALTER TABLE sessions ADD COLUMN draft_context TEXT; ALTER TABLE queue ADD COLUMN context TEXT; PRAGMA user_version = 3;"); })();
}
