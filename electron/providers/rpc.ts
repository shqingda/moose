  ...
  constructor(path: string, args: string[], cwd?: string) {
    ...
    this.child.stdout.on('data', (chunk: string) => {
      ...
      if (this.buffer.length > 16 * 1024 * 1024) {
        this.fail(new Error('Agent response exceeded the protocol buffer limit'));
        void this.close();
        return;
      }
      let end: number;
    ...
    this.child.on('exit', (code, signal) =>
      this.fail(new Error(`Agent disconnected (${signal || code || 'exit'})`)),
    );
    this.child.stdin.on('error', (error) => this.fail(error));
  }
  /** 通过请求 ID 匹配 Promise；无匹配响应交由通知或请求回调处理。 */
