...
it('finds fuzzy files and folders, excludes generated trees and rejects escaping references', async () => {
  ...
  try {
    await mkdir(join(root, '中文 project'));
    await writeFile(join(root, '中文 project', 'Application.tsx'), '');
    await mkdir(join(root, 'node_modules'));
    await writeFile(join(root, 'node_modules', 'secret.ts'), '');
    ...
    expect((await catalog.search(root, '')).some((e) => e.path.includes('node_modules'))).toBe(
      false,
    );
    await expect(
      catalog.resolve(root, { mode: 'build', skills: [], references: ['outside'] }),
...
it('discovers project and global skills and resolves only catalogued IDs', async () => {
  ...
  try {
    ...
    for (const base of [root, project]) {
      const skill = join(base, '.agents/skills/design');
    ...
    const catalog = new ContextCatalog(root),
      entries = await catalog.skills(project);
    expect(entries.map((e) => e.scope)).toEqual(['global', 'project']);
    expect(
      (await catalog.resolve(project, { mode: 'plan', references: [], skills: [entries[1].id] }))
        .skills[0].path,
    ).toContain('/project/');
    await expect(
      catalog.resolve(project, { mode: 'build', references: [], skills: ['unknown'] }),
...
it('waits for a native goal continuation instead of finishing at the first turn', async () => {
  ...
  const root = await mkdtemp(join(tmpdir(), 'moose-goal-')),
    path = resolve('tests/fixtures/agent.mjs');
  await chmod(path, 0o755);
  const adapter = new CodexAdapter(path);
  let text = '';
  try {
    await adapter.run({
      cwd: root,
      ...
      promptContext: { mode: 'goal', references: [], skills: [] },
      nativeId() {},
      emit(event) {
        if (event.kind === 'assistant') text += event.text || event.delta || '';
      },
    });
...
it('removing ordinary inline reference text removes its structured context', async () => {
  ...
  const context = {
    mode: 'build' as const,
    references: ['src/中文 file.ts', 'README.md'],
    skills: ['design'],
  };
  ...
  expect(selected.references).toEqual(['src/中文 file.ts']);
  expect(selected.skills).toEqual(['design']);
  expect(contextInText('ordinary text', selected, skills)).toMatchObject({
    references: [],
    skills: [],
  });
});
