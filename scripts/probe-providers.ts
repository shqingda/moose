...
for (const provider of ['codex', 'grok'] as const) {
  ...
  try {
    ...
    console.log(JSON.stringify({ provider, version: await cliVersion(path), connected: true, models: info.models.map(m => m.id) }));
  } catch (error) { console.error(provider, error instanceof Error ? error.message : error); process.exitCode = 1; }
