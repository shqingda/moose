...
const store = new Store(join(dir, 'moose.sqlite')); store.setSettings({ language: 'en' }); store.addProject(dir); store.close();
...
try {
 ...
 await page.waitForFunction(() => document.querySelector('.sidebar-frame')?.hasAttribute('inert'), undefined, { timeout: 120000 });
