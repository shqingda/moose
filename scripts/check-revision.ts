// 真实 Codex 编辑验收：检查原生 turn / fork 上下文衔接；会产生模型调用。
...
let agent = new CodexAdapter(path),
  answer = '';
try {
  await agent.run({
    session: s,
    ...
    attachments: [
      ...
      },
      {
        ...
        mime: 'text/plain',
        size: 20,
        text: 'Code: MOOSE_CHECKPOINT_427',
      },
    ],
    nativeId: (id) => {
      s.nativeId = id;
  ...
  if (!answer.includes('MOOSE_CHECKPOINT_427') || !turnId)
    throw new Error('Live attachment check failed');
  await agent.close();
  agent = new CodexAdapter(path);
  s.nativeId = await agent.fork(s, cwd, turnId);
  await agent.close();
  agent = new CodexAdapter(path);
  answer = '';
  await agent.run({
    session: s,
    cwd,
    text: 'What exact code did I attach before? Reply with only that code. Do not use tools.',
    nativeId: (id) => {
      s.nativeId = id;
    },
    emit: (event) => {
  ...
  if (!answer.includes('MOOSE_CHECKPOINT_427'))
    throw new Error('Checkpoint continuation lost context');
} finally {
  await agent.close();
}
