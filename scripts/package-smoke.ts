...
execFileSync('/usr/bin/codesign', ['--verify', '--deep', '--strict', resolve('release/mac-arm64/Moose.app')]);
...
const app = await electron.launch({ executablePath: resolve('release/mac-arm64/Moose.app/Contents/MacOS/Moose'), env: { ...env, MOOSE_DATA_DIR: dir } });
try {
 ...
 if (details.version !== '0.5.5' || !details.sandbox || !Array.isArray(snapshot.projects)) throw new Error('Packaged smoke failed');
 await page.screenshot({ path: 'test-results/package-0.5.5.png' }); console.log(JSON.stringify({ ...details, sqlite: 'ready' }));
